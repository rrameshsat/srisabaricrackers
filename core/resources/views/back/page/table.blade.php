@foreach ($datas as $data)
    <tr>
        ...
        <td>
            @php
                $showInMenu = \Illuminate\Support\Facades\Schema::hasColumn('pages', 'show_in_menu') ? (int) $data->show_in_menu : 1;
            @endphp
            <div class="dropdown">
                <button class="btn btn-{{ $showInMenu ? 'success' : 'danger' }} btn-sm btn-rounded dropdown-toggle"
                    type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ $showInMenu ? __('Enabled') : __('Disabled') }}
                </button>
                <div class="dropdown-menu animated--fade-in">
                    <a class="dropdown-item" href="{{ route('back.page.menu.status', [$data->id, 1]) }}">{{ __('Enabled') }}</a>
                    <a class="dropdown-item" href="{{ route('back.page.menu.status', [$data->id, 0]) }}">{{ __('Disabled') }}</a>
                </div>
            </div>
        </td>
        <td>
