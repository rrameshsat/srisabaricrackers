...
<script>
    (function($){
        ...
        $(function(){
            // Initialize for billing
            var $countryBilling = $('#billing-country');
            window.userStateId = {{ isset($user) ? $user->bill_state_id : 'null' }};
            loadStatesForCountry($countryBilling, $('#billing-state-block'), $('#billing-state'));
            $countryBilling.change(function(){ loadStatesForCountry(this, $('#billing-state-block'), $('#billing-state')); });
            // Auto-select India if available and no selection made (tighten UI)
            if ($countryBilling.find('option[value="India"]').length > 0 && $countryBilling.val() === '') {
                $countryBilling.val('India').trigger('change');
            }

            // Initialize for shipping if present
            var $countryShipping = $('#shipping-country');
            if ($countryShipping.length) {
                    ...
                    <div class="card-body">
                        ...
                        <form id="checkoutBilling" action="{{ route('front.checkout.store') }}" method="POST">
                            ...
                            @if (PriceHelper::CheckDigital())
                                ...
                                <div class="col-sm-12" id="billing-state-block" style="display:none;">
                                    <div class="form-group">
                                        <label for="billing-state">{{ __('State (include tax) : Select Billing State') }}</label>
                                        <select class="form-control" name="bill_state_id" id="billing-state">
                                            <option value="">{{ __('Select Billing State') }}</option>
                                        </select>
                                    </div>
                                </div>
                                </div>
