...
class ImageHelper
{
    public static function handleUploadedImage($file, $path, $delete = null)
    {
        if ($file) {
            ...
            if ($delete) {
                Storage::disk('public_legacy')->delete($path . '/' . $delete);
            }

            $name = Str::random(4) . $file->getClientOriginalName();
            Storage::disk('public_legacy')->putFileAs($path, $file, $name);
    ...
    public static function ItemhandleUploadedImage($file, $path, $delete = null)
    {
        if ($file) {
            ...
            Storage::disk('public_legacy')->putFileAs($path, $file, $photoName);
            ...
            Storage::disk('public_legacy')->put($thumbnailPath, (string) $image->encode());
    ...
    public static function handleUpdatedUploadedImage($file, $path, $data, $delete_path, $field)
    {
        ...
        if ($data[$field] != null) {
            Storage::disk('public_legacy')->delete($delete_path . '/' . $data[$field]);
        }
    ...
    public static function ItemhandleUpdatedUploadedImage($file, $path, $data, $delete_path, $field)
    {
        ...
        if (!empty($data['thumbnail'])) {
            Storage::disk('public_legacy')->delete($delete_path . '/' . $data['thumbnail']);
        }
