$(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".decreaseQtycart", function () {
            ...
            if (qty > 0) {
                getData(1, item_key, item_id, qty, 0, 0, 'minus');
            }
        });
        ...
        function getData(status = 0, check = 0, item_key = 0, qty = 0, add_type = 0, optionIds = null, isIncrement = 0) {
            ...
            if (status == 1) {
                let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}&options_ids=${options_ids}&attribute_ids=${attribute_ids}&quantity=${quantity}&type=${type}&item_key=${item_key}&add_type=${add_type}&isIncrement=${isIncrement}`;
                $.ajax({
                    method: "GET",
                    contentType: false,
                    cache: false,
