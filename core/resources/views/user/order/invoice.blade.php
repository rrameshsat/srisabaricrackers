                            ...
                            <tbody>
                                ...
                                @foreach (json_decode($order->cart, true) as $key => $item)
                                    @php
                                        ...
                                        $total += \App\Helpers\PriceHelper::cartEntryUnitPrice($item) * $item['qty'];
                                        $option_price += empty($item['tax_exempt']) && empty($item['quick_shopping']) ? $item['attribute_price'] : 0;
                                        $grandSubtotal = $total + $option_price;
                                        if (App\Models\Item::where('id', \App\Helpers\PriceHelper::GetItemId($key))->exists()) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-0 text-right">
                                            @php
                                                $total_price = \App\Helpers\PriceHelper::cartEntryTotalPrice($item) * $item['qty'];
                                            @endphp
