...
<head>
    ...
    @php
        $faviconUrl = null;
        if ($setting->favicon) {
            $faviconUrl = str_contains($setting->favicon, '/')
                ? asset('storage/' . $setting->favicon)
                : url('/core/public/storage/images/' . $setting->favicon);
        } else {
            $faviconUrl = url('/core/public/storage/images/placeholder.png');
        }
    @endphp
            ...
            <div class="content">
                <div class="page-inner">
                    <div class="content-wrapper">
                        @php
                            $securityData = Session::get('securityData');
                        @endphp
                        @if ($securityData && $securityData['status'] == 'not_verified')
                            <div class="alert alert-danger">
                                <h3><b>{{ __('Your domain is not verified.') }}</b></h3>
                                <p>
                                    <b>Contact Support : </b> <a href="https://support.geniusdevs.com/"
                                        target="_blank">https://support.geniusdevs.com/</a>
                                </p>
                            </div>
                        @elseif($securityData && $securityData['status'] == 'multiple_domain')
                            <div class="alert alert-danger">
                                <h3><b>{{ __('Multiple domain is not allowed.') }}</b></h3>
                                <ul>
                                    @foreach ($securityData['data'] as $data)
                                        <li>{{ $data['domin_url'] }}</li>
                                    @endforeach
                                </ul>
                                <p>
                                    <b>Contact Support : </b> <a href="https://support.geniusdevs.com/"
